...
<body>
    ...
    <div class="main"><span class="categories"> Novels </span><br>
        <div class="books" style="display: flex; ">
            ...
            $con = mysqli_connect("localhost", "root", "");
            mysqli_select_db($con, "shopping cart");
            $query = "SELECT  `name`, `image`, `price`, `discount` FROM `product items` order by id ASC ";
            $queryfire = mysqli_query($con, $query);
            $num = mysqli_num_rows($queryfire);
            if ($num > 0) {
                while ($product = mysqli_fetch_array($queryfire)) {
            ?>
                    <div class="cover">
                        ...
                        <div class="info">
                            <dt><?php echo $product['name']; ?>
                                <dl><?php echo $product['price']; ?></dl>
                                <dl> <?php echo $product['discount']; ?></dl>
                            </dt>
